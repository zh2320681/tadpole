...
class EventHandler {
	 ...
	 */
	public Object handleEvent(ZEvent event) throws InvocationTargetException {
		if (!valid) {
		...
		for (int i = 0; i < paraTypes.length; i++) {
			...
			//如果是event类型
			if(ZEvent.class.isAssignableFrom(paraType)){
				objs[i] = event;
				continue;
			}
...
//				objs[i] = ZInjector.instance(null).getDefaultInstance(paraType);
//				continue;
//			}
			objs[i] = null;
		}
